...
def main():
    ...
    np.random.seed(123)
    s.run(1000000, Graph(n))

    edge_sizes = [np.sum(np.array(list(st), dtype=int)) for st in s.res['SAMPLES'][100000:]]
    edge_adjm = str_list_to_adjm(n, s.res['SAMPLES'][100000::])
    ...
    np.random.seed(123)
    s_.run(1000000, Graph(n))

    triangle_sizes = [np.sum(np.array(list(st), dtype=int)) for st in s_.res['SAMPLES'][100000:]]
    triangle_adjm = str_list_to_adjm(n, s_.res['SAMPLES'][100000:])
